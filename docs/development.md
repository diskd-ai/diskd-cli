...
```sh
gh workflow run release --repo diskd-ai/diskd-cli --field version=v0.1.1 --ref main
```

Verify:

```sh
gh release view v0.1.1 --repo diskd-ai/diskd-cli
```
...
```sh
tmp="$(mktemp -d)"
DISKD_INSTALL_DIR="$tmp" DISKD_VERSION=v0.1.1 sh install.sh
