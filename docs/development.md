...
```sh
gh workflow run release --repo diskd-ai/diskd-cli --field version=v0.1.5 --ref main
```

Verify:

```sh
gh release view v0.1.5 --repo diskd-ai/diskd-cli
```
...
```sh
tmp="$(mktemp -d)"
DISKD_INSTALL_DIR="$tmp" DISKD_VERSION=v0.1.5 sh install.sh
